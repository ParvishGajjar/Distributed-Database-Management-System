...
//        }
    ...
    private static void beautifyOutput(String data, String tableName) {
        List<String> cols = getColumnsFromTable(tableName, Utils.currentDbName, true);
    ...
    public static List<String> getColumnsFromTable(String tableName, String dbName, boolean shouldRequestVm) {
        ArrayList<String> res = new ArrayList<>();
        try {
            if(ExportEngine.isLocalTable(tableName,Utils.currentDbName)) {
                BufferedReader br = new BufferedReader(new FileReader(Utils.resourcePath + Utils.currentDbName + "/" + tableName + ".tsv"));
                br.readLine();
                ...
                return Requester.getInstance().requestVmGetColumns(tableName, Utils.currentDbName);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    static void listQuery(String query) {
        try {
            if (query.contains("table")) {
                if (Utils.currentDbName != null && Utils.currentDbName.length() != 0) {
                    ...
                    System.err.println("select a database by 'use database command' and then try show tables");
                    try {
                ...
                if(lst.size()>0) {
                    ...
                    System.out.println("no databases available to show");
            }
        }catch (Exception e){
            LogManager.writeCrashReportsToEventLogs(e.getLocalizedMessage());
        }
